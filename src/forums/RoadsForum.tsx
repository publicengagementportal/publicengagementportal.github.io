...
const RoadsForum = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const postsRef = collection(db, 'roadsForumPosts');
  ...
  const handlePostSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, 'roadsForumPosts'), newPost);
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="w-2/3 p-6">
        <h1 className="text-3xl font-bold mb-4">Roads Forum</h1>
        <p className="mb-4">Welcome to the Roads discussion forum. Share your thoughts and ideas about roads here.</p>
        <button
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <form onSubmit={handlePostSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-700">Severity of Damage:</label>
              <select 
                name="severity" 
                required
                className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
